...
async def add_owner(
    ...
    await owner_facade.add_hospital_owner(Model=Model, session=session)
...
async def get_all_owners(
    ...
    owners = await owner_facade.get_all_hospital_owners(session=session)
    if not owners:
        raise HTTPException(
...
async def update_owner(
    ...
    owner: Owner = await owner_facade.get_hospital_owner(owner_id=owner_id, session=session)
    ...
    updated_owner = await owner_facade.update_hospital_owner(owner_id=owner_id, Model=Model, session=session)
...
async def delete_owner(
    ...
    await owner_facade.delete_hospital_owner(owner_id=owner_id, session=session)
    await user_facade.delete_user(user_id=owner_id, session=session)
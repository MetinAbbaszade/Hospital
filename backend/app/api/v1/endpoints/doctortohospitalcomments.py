...
from app.service.doctortohospitalcomment import Facade as DH_facade
...
async def create_doctor_to_hospital_comment(Model: PostDoctorToHospitalCommentModel, session: AsyncSession = Depends(get_db)):
    ...
    hospitalcomment = await dh_facade.add_dh_comment(Model=Model, session=session)
...
async def get_doctor_to_hospital_comments(session: AsyncSession = Depends(get_db)):
    hospitalcomments = await dh_facade.get_all_dh_comments(session=session)
...
async def get_doctor_to_hospital_comment(hospitalcomment_id: UUID | str, session: AsyncSession = Depends(get_db)):
    hospitalcomment = await dh_facade.get_dh_comment(doctortohospitalcomment_id=hospitalcomment_id, session=session)
...
async def get_dh_comment_by_hospital_id(hospital_id: UUID, session: AsyncSession = Depends(get_db)):
    existing_hospital = await hospital_facade.get_hospital(hospital_id=hospital_id, session=session)
    if not existing_hospital:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail='Hospital not found'
    ...
    comments = await dh_facade.get_dh_comment_by_hospital_id(hospital_id=hospital_id, session=session)
    if not comments:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def get_dh_comment_by_doctor_id(doctor_id: UUID, session: AsyncSession = Depends(get_db)):
    ...
    comments = await dh_facade.get_dh_comment_by_doctor_id(doctor_id=doctor_id, session=session)
    if not comments:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def update_doctor_to_hospital_comment(hospitalcomment_id: UUID | str, Model: UpdateDoctorToHospitalCommentModel, session: AsyncSession = Depends(get_db)):
    existing_hospitalcomment = await dh_facade.get_dh_comment(doctortohospitalcomment_id=hospitalcomment_id, session=session)
    ...
    hospitalcomment = await dh_facade.update_dh_comment(Model=Model, doctortohospitalcomment_id=hospitalcomment_id, session=session)
    return hospitalcomment
...
async def delete_doctor_to_hospital_comment(hospitalcomment_id: UUID | str, session: AsyncSession = Depends(get_db)):
    ...
    await dh_facade.delete_dh_comment(doctortohospitalcomment_id=hospitalcomment_id, session=session)
    return None
...
async def delete_dh_comment_by_hospital_id(hospital_id: UUID, session: AsyncSession = Depends(get_db)):
    existing_hospitalcomment = await dh_facade.get_dh_comment_by_hospital_id(hospital_id=hospital_id, session=session)
    if not existing_hospitalcomment:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail='Hospitalcomment not found'
        )
    await dh_facade.delete_dh_comment_by_hospital_id(hospital_id=hospital_id, session=session)
    return None
...
async def delete_dh_comment_by_doctor_id(doctor_id: UUID, session: AsyncSession = Depends(get_db)):
    existing_doctorcomment = await dh_facade.get_dh_comment_by_doctor_id(doctor_id=doctor_id, session=session)
    if not existing_doctorcomment:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail='Hospitalcomment not found'
        )
    await dh_facade.delete_dh_comment_by_doctor_id(doctor_id=doctor_id, session=session)
    return None